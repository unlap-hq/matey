...
from matey.core import ResolvedPaths, SchemaValidationError, ScratchProvisionError
from matey.core.diff import (
    _normalize_sql,
    _read_text,
    _schema_diff,
    normalize_sql_text,
...
def _run_check_on_scratch(
    *,
    ...
    retry_count = scratch_retry_count(
        engine=engine,
    ...
    try:
        ready_error = ensure_scratch_ready(
            ...
            wait_error=f"dbmate wait failed on {check_name} scratch target.",
            create_error=f"dbmate create failed on {check_name} scratch target.",
        ...
        cleanup_error = cleanup_scratch(
            ...
            on_dbmate_result=on_dbmate_result,
            drop_error="dbmate drop failed while cleaning scratch target.",
            run_dbmate_fn=run_dbmate,
        )
        if cleanup_error:
            if error_message is None:
                ...
                error_message = f"{error_message} Cleanup error: {cleanup_error}"
