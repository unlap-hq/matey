...
class DbEngine:
    ...
    def db_plan_sql(self, *, runtime: TargetRuntime, defaults: ConfigDefaults) -> str:
    ...
    def _run_guarded_mutation(
        ...
        *,
        ...
        with self._ctx.scope.open(target_key=op.target_key, target_root=op.target_paths.db_dir):
            ...
            expected_index = pre_index + expected_delta
            if expected_index < 0:
                expected_index = 0
    ...
    def _load_lock(self, *, runtime: TargetRuntime) -> SchemaLock:
    ...
    def _empty_baseline_sql(self, *, op: DbOpContext) -> str:
        ...
        try:
            ...
            if scratch.cleanup_required and not op.keep_scratch:
                ...
                if drop.exit_code != 0:
                    raise ExternalCommandError((drop.stderr or drop.stdout or "dbmate drop failed").strip())
