...
from matey.workflows.lockfile import SchemaLock, doctor_schema_lock
from matey.workflows.schema import dump_schema_for_url
from matey.workflows.schema_diff import normalize_sql_text, schema_diff_text
from matey.workflows.schema_lock import expected_schema_from_head_lock
...
def _live_applied_index(
    *,
    ...
    if status_result.returncode != 0:
        ...
        raise SchemaValidationError(f"dbmate status failed on live target. {details}".strip())
...
def _live_applied_index_with_create_if_needed(
    *,
    ...
    try:
        ...
        except SchemaValidationError as status_after_create_error:
            if create_code != 0:
                raise SchemaValidationError(
