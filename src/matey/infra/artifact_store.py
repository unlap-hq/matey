...
class SqliteArtifactStore(IArtifactStore):
    ...
    def _resolve_inside(root: Path, rel_path: str) -> Path:
        ...
        try:
            ...
            raise ArtifactTransactionError(f"Path escapes target root: {rel_path}") from error
        return candidate
    ...
    def _copy_file(src: Path, dst: Path) -> None:
        ...
        SqliteArtifactStore._fsync_file(dst)
        SqliteArtifactStore._fsync_directory(dst.parent)
    ...
    def _write_bytes(path: Path, payload: bytes) -> None:
        ...
        with tmp.open("wb") as handle:
            ...
            os.fsync(handle.fileno())
        tmp.replace(path)
        SqliteArtifactStore._fsync_directory(path.parent)
    ...
    def begin(
        ...
        *,
        ...
        self._fsync_directory(staged_root)
        ...
        for item in writes:
            target_path = self._resolve_inside(root, item.rel_path)
            staged_path = staged_root / item.rel_path
            self._write_bytes(staged_path, item.content)
        ...
        self._fsync_directory(staged_root)
        self._fsync_directory(backup_root)
        self._fsync_directory(tx_root)
        self._fsync_directory(self._tx_dir)
    ...
    def apply(self, *, txid: str) -> None:
        ...
        try:
            for row in writes:
                target_path = self._resolve_inside(root, row.rel_path)
                staged_path = Path(row.staged_path)
                if not staged_path.exists():
                    self._raise_missing_staged(txid=txid, rel_path=row.rel_path)
                self._write_bytes(target_path, staged_path.read_bytes())
        ...
        for row in writes:
            ...
            if row.backup_path is not None:
                ...
                if backup_path.exists():
                    self._write_bytes(target_path, backup_path.read_bytes())
