...
from matey.contracts import IDbmateGateway, IEnvProvider, IProcessRunner
...
def default_dbmate_binary() -> Path:
    vendor_root = Path(__file__).resolve().parent / "_vendor" / "dbmate"
...
class DbmateGateway(IDbmateGateway):
    ...
    def _run_dbmate(
        ...
        *,
        ...
        exec_cwd = cwd if cwd is not None else migrations_dir.parent
        return self._runner.run(tuple(argv), cwd=exec_cwd)
    ...
    def dump(self, url: str, migrations_dir: Path) -> CmdResult:
        with tempfile.TemporaryDirectory(prefix="matey-dbmate-dump-") as tmp_name:
            out_path = Path(tmp_name) / "schema.sql"
            result = self._run_dbmate(
                url=url,
                migrations_dir=migrations_dir,
                verb="dump",
                no_dump_schema=False,
                global_args=("--schema-file", str(out_path)),
                cwd=migrations_dir.parent,
            )
            dump_text = out_path.read_text(encoding="utf-8") if out_path.exists() else ""
            return CmdResult(
                argv=result.argv,
