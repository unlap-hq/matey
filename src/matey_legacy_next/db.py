...
class DbEngine:
    ...
    def db_raw(
        ...
        *,
        ...
        with self._target_scope(runtime):
            url = self._resolve_live_url(runtime=runtime, url_override=url_override)
            result = self._ctx.dbmate.raw(
                argv_suffix=argv_suffix, url=url, migrations_dir=runtime.paths.migrations_dir
            )
    ...
    def db_plan_sql(self, *, runtime, defaults: ConfigDefaults) -> str:
        status = self._schema_engine.schema_status(
            runtime=runtime, defaults=defaults, base_ref=None
        )
        if status.stale:
            raise SchemaMismatchError("schema status is stale; run schema apply before db plan sql")
    ...
    def _run_guarded_mutation(
        ...
        *,
        ...
        with self._ctx.scope.open(target_key=op.target_key, target_root=op.target_paths.db_dir):
            ...
            if verb == "up":
                mutate = self._ctx.dbmate.up(
                    url=op.live_url, migrations_dir=op.target_paths.migrations_dir
                )
            ...
            expected_index = (
                len(lock.steps) if verb in {"up", "migrate"} else max(0, pre_index - steps)
            )
    ...
    def _run_db_compare(
        ...
        *,
        ...
        with self._ctx.scope.open(target_key=op.target_key, target_root=op.target_paths.db_dir):
            ...
            live_dump = self._ctx.dbmate.dump(
                url=op.live_url, migrations_dir=op.target_paths.migrations_dir
            )
            ...
            if mode == "current":
                expected_sql = self._expected_sql_for_index(
                    op=op, lock=lock, index=snapshot.applied_count
                )
            elif mode == "head":
            ...
            return DbPlanOutput(
                result=DbPlanResult(
                    comparison=comparison, live_applied_index=snapshot.applied_count
                ),
    ...
    def _target_scope(self, runtime):
        return self._ctx.scope.open(
            target_key=derive_target_key(
                repo_root=self._ctx.git.repo_root(), db_dir=runtime.paths.db_dir
            ),
    ...
    def _build_db_context(
        ...
        *,
        ...
        test_url = normalized_optional(test_url_override) or normalized_optional(
            self._ctx.env.get(runtime.test_url_env)
        )
        op = DbOpContext(
    ...
    def _resolve_live_url(self, *, runtime, url_override: str | None) -> str:
        live_url = normalized_optional(url_override) or normalized_optional(
            self._ctx.env.get(runtime.url_env)
        )
        if live_url is None:
    ...
    def _preflight_status(
        self, *, op: DbOpContext, lock: SchemaLock, verb: str
    ) -> DbStatusSnapshot:
        ...
        status_result = self._ctx.dbmate.status(
            url=op.live_url, migrations_dir=op.target_paths.migrations_dir
        )
    ...
    def _require_lock_prefix(*, lock: SchemaLock, snapshot: DbStatusSnapshot) -> None:
        ...
        normalized_applied = tuple(
            file if "/" in file else f"migrations/{file}" for file in raw_applied
        )
        expected_prefix = tuple(
            step.migration_file for step in lock.steps[: len(normalized_applied)]
        )
    ...
    def _expected_sql_for_index(self, *, op: DbOpContext, lock: SchemaLock, index: int) -> str:
        ...
        if not checkpoint_path.exists():
            raise LockfileError(
                f"Expected checkpoint not found for index {index}: {checkpoint_path}"
            )
    ...
    def _empty_baseline_sql(self, *, op: DbOpContext) -> str:
        ...
        try:
            ...
            create = self._ctx.dbmate.create(
                url=scratch.url, migrations_dir=op.target_paths.migrations_dir
            )
            require_cmd_success(create, "dbmate create failed for baseline scratch")
            dump = self._ctx.dbmate.dump(
                url=scratch.url, migrations_dir=op.target_paths.migrations_dir
            )
            ...
            if not op.keep_scratch:
                if scratch.cleanup_required:
                    drop = self._ctx.dbmate.drop(
                        url=scratch.url, migrations_dir=op.target_paths.migrations_dir
                    )
                    if drop.exit_code != 0:
                        cleanup_errors.append(
                            (drop.stderr or drop.stdout or "dbmate drop failed").strip()
                        )
                try:
